  ...
  return (
    <div className='flex items-center justify-between py-5 font-medium'>
      ...
      <Link to='/Cart' className='relative'>
            ...
            <p className='absolute right-[-5px] bottom-[-5px] w-4 text-center bg-black text-white rounded-full text-[8px]'>{getCartCount()}</p>
      </Link>
      <img onClick={()=>setVisible(true)} src={assets.menu_icon} className='w-5 coursor-pointer sm:hidden ' alt="" />
    </div>
   {/* create mane bar for small screen*/}
   ...
   <div className={`absolute top-0 bottom-0 overflow-hidden bg-white transition-all ${visible ? 'w-full' :'w-0'}` }>
        <div className='flex flex-col text-gray-600'>
          ...
          <NavLink  onClick={()=>{setVisible(false)}} className='py-2 pl-6 border' to='/'> ABOUT</NavLink>  
